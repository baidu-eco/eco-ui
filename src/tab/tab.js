...
define('common:udo/ui/tab', function(require, exports, module) {
	...
	pri.bindEvent = function() {
		...
		this.elems.context.delegate(this.def.selector, this.def.eventType + '.tab', function(event) {

			// 已经点击过的不再次触发点击功能
			if( !$(this).hasClass(classes.clicked) ) {

				var params = pri.getParams.call(_this, this);

				params.clicked.addClass(classes.clicked);
				params.other_clicked.removeClass(classes.clicked);

				_this.onclick(params);
			}
		...
		// 默认触发
		this.elems.handlers.eq(this.def.index).trigger(this.def.eventType + '.tab');
	};

	// 获取点击的属性，以及对应的内容
	pri.getParams = function(target) {
	...
	// 获取当前点击项对应的内容元素
	pri.findCurrent = function(list, s) {
	...
	fn.ajaxFun = function(params, action) {
